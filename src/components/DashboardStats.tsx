...
export default function DashboardStats({ stats, loading }: DashboardStatsProps) {
  ...
  if (loading || !stats) {
    return (
      <div id="dashboard-stats-skeleton" className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-pulse">
        {[1, 2, 3].map((idx) => (
          <div key={idx} className="bg-white p-6 rounded-3xl border border-[#2D6A4F]/10 shadow-premium space-y-4">
            <div className="flex justify-between items-start">
              <div className="h-10 w-10 bg-stone-100 rounded-2xl" />
            ...
            <div className="h-3 w-40 bg-stone-50 rounded-full pt-1" />
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div id="dashboard-stats-grid" className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-[#2D6A4F]/10 shadow-premium flex flex-col justify-between transition-all duration-300 hover:shadow-[#2D6A4F]/10 relative group overflow-hidden">
        ...
        <div className="flex items-start justify-between">
          ...
          <span className="bg-emerald-50 text-emerald-800 text-[10px] font-mono font-bold tracking-wider uppercase px-2.5 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="mt-4">
          ...
          <h3 className="text-2xl font-serif font-extrabold text-[#1B4332] mt-1 tracking-tight">
            {formatCurrency(activeInvestmentsValue)}
          </h3>
          <p className="text-[11px] text-gray-500 mt-2 font-sans flex items-center gap-1.5">
            <span className="font-bold text-[#2D6A4F]">{activeInvestmentsCount}</span> deeded plot active holdings configured
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-[#2D6A4F]/10 shadow-premium flex flex-col justify-between transition-all duration-300 hover:shadow-[#2D6A4F]/10 relative group overflow-hidden">
        ...
        <div className="flex items-start justify-between">
          ...
          <span className="bg-amber-50 text-amber-800 text-[10px] font-mono font-bold tracking-wider uppercase px-2.5 py-1 rounded-full flex items-center gap-1">
      ...
      <div className="bg-white p-6 rounded-3xl border border-[#2D6A4F]/10 shadow-premium flex flex-col justify-between transition-all duration-300 hover:shadow-[#2D6A4F]/10 relative group overflow-hidden">
        ...
        <div className="flex items-start justify-between">
          <div className="p-3 bg-[#52B788]/10 text-emerald-800 rounded-2xl">
            <Sprout className="h-6 w-6" />
          </div>
          <span className="bg-[#52B788]/10 text-emerald-800 text-[10px] font-mono font-bold tracking-wider uppercase px-2.5 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="mt-4">
          ...
          <h3 className="text-2xl font-serif font-extrabold text-[#1B4332] mt-1 tracking-tight">
            {activeFarms} <span className="text-xs text-gray-400 font-sans font-normal">/ {totalFarms} Configured</span>
          </h3>
          <p className="text-[11px] text-gray-500 mt-2 font-sans">
